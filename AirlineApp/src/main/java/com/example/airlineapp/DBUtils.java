...
public class DBUtils {
			...
			if (ticket.getFlightID() == null) {
				...
				try {
					...
					preparedStatement = connection.prepareStatement(
							"SELECT depart_time, arrival_time,origin,destination,flightdate,aircraft FROM flights WHERE flightID =?");
	...
	public static void changeSceneAfterPay(ActionEvent event, String fxmlFile, String title, Ticket ticket,
			String firstname, String lastname, String email, String cardholdername, String cardnumber,
			String cardcode) {
		Parent root = null;
		...
		try {
			...
			psInsert = connection.prepareStatement(
					"INSERT INTO tickets (ticketID,paymentID,flightID,cust_lastname,cust_firstname,seatID,price,depart_time,arrival_time,originOutput,destinationOutput,flightdate,aircraft,email) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			psInsertPayment = connection.prepareStatement(
	...
	public static void signUpUser(ActionEvent event, User userObject) {
		...
		try {
			...
			if (resultSet.isBeforeFirst()) {
				...
				psInsert = connection.prepareStatement(
						"INSERT INTO users (username,password,usertype,email,lastname,firstname) VALUES(?,?,?,?,?,?)");
	...
	public static void logInUser(ActionEvent event, String username, String password) {
		// this function is on the login page and click on login after the username and
		// password is entered.
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
	...
	public static void searchFlight(ActionEvent event, ListView<String> listview_box, Label label_select, String origin,
			String destination, Ticket ticket) {
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		DatabaseConnection connectionObject = new DatabaseConnection();
		...
		try {
			// connection =
			// DriverManager.getConnection("jdbc:mysql://localhost:3306/AirlineApp", "root",
			// "12345678");
			connection = connectionObject.getDBConnection();
			preparedStatement = connection.prepareStatement(
					"SELECT origin, destination,flightdate FROM flights WHERE origin =? AND destination=? AND flightdate=?");
			...
			if (!resultSet.isBeforeFirst()) {
				...
				preparedStatement = connection.prepareStatement(
						"SELECT flightID, depart_time, arrival_time, origin,destination,flightdate,aircraft FROM flights WHERE origin =? AND destination=? AND flightDate=?");
				preparedStatement.setString(1, origin);
				preparedStatement.setString(2, destination);
				preparedStatement.setString(3, ticket.getFlightdate());
				resultSet = preparedStatement.executeQuery();
				...
				while (resultSet.next()) {
					...
					String listOut = flightID + "\t\t" + depart_time + "\t\t" + arrival_time + "\t\t" + originOutput
							+ "\t\t" + destinationOutput + "\t\t" + flightdate + "\t\t" + aircraft;
				...
				listview_box.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
					...
					public void changed(ObservableValue<? extends String> observableValue, String s, String t1) {
						...
						if (t1 != null) {// this if{} is added to handle clear the listview_box after each time search
											// is pressed.
	...
	public static Ticket getTicketObject(String ticketID) throws SQLException, IOException {
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		DatabaseConnection connectionObject = new DatabaseConnection();
		...
		connection = connectionObject.getDBConnection();
		preparedStatement = connection.prepareStatement(
				"SELECT paymentID, flightID,cust_lastname,cust_firstname,seatID,price,depart_time,arrival_time,originOutput,destinationOutput,flightdate,aircraft,email FROM tickets WHERE ticketID =?");
	...
	public static User getUserObject(String username) throws SQLException, IOException {
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		DatabaseConnection connectionObject = new DatabaseConnection();
		...
		connection = connectionObject.getDBConnection();
		preparedStatement = connection.prepareStatement(
				"SELECT usertype, email,lastname,firstname,discountcode FROM users WHERE username =?");
		preparedStatement.setString(1, username);
		resultSet = preparedStatement.executeQuery();
	...
	public static void cancelFlight(ActionEvent event, String fxmlFile, String ticketID, String paymentID) {
		...
		try {
			...
			preparedStatement = connection
					.prepareStatement("SELECT ticketID,paymentID FROM tickets WHERE ticketID =? AND paymentID=? ");
			preparedStatement.setString(1, ticketID);
			...
			if (!resultSet.isBeforeFirst()) {
				...
				if (result.isPresent() && result.get() == ButtonType.OK) {
					...
					preparedStatement = connection
							.prepareStatement("DELETE FROM tickets WHERE ticketID =? AND paymentID=? ");
					preparedStatement.setString(1, ticketID);
					preparedStatement.setString(2, paymentID);
	...
	public static void displayTickets(ListView<String> listview_box, User user, Ticket outsideTicketObject) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				String listOut = ticket.getTicketID() + "    " + ticket.getFlightID() + "    "
						+ ticket.getOriginOutput() + "    " + ticket.getDestinationOutput() + "    "
						+ ticket.getFlightdate() + "    " + ticket.getDepart_time() + "    " + ticket.getArrival_time();
				listview_box.getItems().add(listOut);// populate listview with values
			}
