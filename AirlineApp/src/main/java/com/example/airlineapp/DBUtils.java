...
public class DBUtils {
	...
	try {
	    ...
	    connection = connectionObject.getDBConnection();
	    psInsert = connection.prepareStatement("INSERT INTO tickets (ticketID,paymentID,flightID,cust_lastname,cust_firstname,seatID,price,depart_time,arrival_time,originOutput,destinationOutput,flightdate) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void changeSceneToSearch(ActionEvent event, String fxmlFile, String title) {
	Parent root = null;
...
//	    connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/AirlineApp", "root", "12345678");
	    connection = connectionObject.getDBConnection();
	    preparedStatement = connection.prepareStatement("SELECT origin, destination FROM flights WHERE origin =? AND destination=?");
		...
		preparedStatement = connection.prepareStatement("SELECT flightID, depart_time, arrival_time, origin,destination,flightdate,aircraft FROM flights WHERE origin =? AND destination=?");
		preparedStatement.setString(1, origin);
		preparedStatement.setString(2, destination);
		resultSet = preparedStatement.executeQuery();
